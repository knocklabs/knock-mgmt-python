...
While you can provide a `service_token` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `KNOCK_SERVICE_TOKEN="My Service Token"` to your `.env` file
so that your Service Token is not stored in source control.

## Async usage

Simply import `AsyncKnockMgmt` instead of `KnockMgmt` and use `await` with each API call:
...
List methods in the Knock Mgmt API are paginated.
...
for workflow in client.workflows.list(
    ...
    # Do something with workflow here
...
```python
first_page = await client.workflows.list(
    environment="development",
)
...
print(f"next page cursor: {first_page.page_info.after}")  # => "next page cursor: ..."
for workflow in first_page.entries:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `knock_mapi.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `knock_mapi.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `knock_mapi.APIError`.
...
except knock_mapi.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except knock_mapi.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except knock_mapi.APIStatusError as e:
...
client.with_options(max_retries=5).workflows.list(
    environment="development",
)
```
...
client.with_options(timeout=5.0).workflows.list(
    environment="development",
)
```
...
You can enable logging by setting the environment variable `KNOCK_MGMT_LOG` to `info`.

```shell
$ export KNOCK_MGMT_LOG=info
```
...
workflow = response.parse()  # get the object that `workflows.list()` would have returned
print(workflow.valid)
```
...
```python
import httpx
from knock_mapi import KnockMgmt, DefaultHttpxClient

client = KnockMgmt(
    # Or use the `KNOCK_MGMT_BASE_URL` env var
