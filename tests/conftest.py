...
logging.getLogger("knock_mapi").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[KnockMgmt]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with KnockMgmt(base_url=base_url, service_token=service_token, _strict_response_validation=strict) as client:
        yield client
